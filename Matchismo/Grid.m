...
{
    CGRect frame = CGRectMake(-CELL_SPACING * 2, 0, self.cellSize.width - CELL_SPACING, self.cellSize.height - CELL_SPACING);
    frame.origin.x += column * (self.cellSize.width + CELL_SPACING);
    frame.origin.y += row * (self.cellSize.height + CELL_SPACING);
    return frame;
}

- (NSInteger)exactNumberOfCells
{
